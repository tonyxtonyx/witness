...
const key=import.meta.env.VITE_API_KEY||'dev-secret';
export async function api<T>(path:string,init?:RequestInit):Promise<T>{
  const res=await fetch('/api/v1'+path,{...init,headers:{'X-API-Key':key,'Content-Type':'application/json',...init?.headers}});
